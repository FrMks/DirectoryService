            ...
            Result<DepartmentDtoWithPreloadingChildren[], Errors>>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var departments = await GetRootSectionsWithPreloadingChildrenCachedAsync(page, pageSize, offset, prefetch, cancellationToken);

        return Result.Success<DepartmentDtoWithPreloadingChildren[], Errors>(departments);
    }

    private async Task<DepartmentDtoWithPreloadingChildren[]> GetRootSectionsWithPreloadingChildrenCachedAsync
    (
        int page,
        int pageSize,
        int offset,
        ...
        CancellationToken cancellationToken)
    {
        var cacheKey = $"root_sections_with_preloading_children_page_{page}_size_{pageSize}_prefetch_{prefetch}";
